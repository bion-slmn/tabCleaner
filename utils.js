        ...
        if (!tab.audible && !tab.active && !tab.pendingUrl && !tab.discarded) {
            chrome.tabs.discard(tabId, () => {
                if (chrome.runtime.lastError) {
                    console.error(`Error discarding tab ${tabId}: ${chrome.runtime.lastError.message}`);
                } else {
                    console.log(`Tab ${tabId} has been discarded.`);
                }
            });
        } else {
            console.log(`Tab ${tabId} cannot be discarded. It may be playing audio/video, downloading, or active.`);
        }
    });
}
